...
def compute_new(z1, z2, up):
    ...
    v = (z2-z1) /2
    v_mod, v_phas = cmath.polar(v)
    v_mod *= np.sqrt(2)
    
    return cmath.rect(v_mod, v_phas)*1j**0.5

def partial_Levi_curve(T):
    '''
...
def Levi_curve(n):
    ...
    if n==0 :
        ...
        return partial_Levi_curve(Levi_curve(n-1))
...
if __name__ == '__main__' :
    ...
    def plot(C_List):
        Lr = [el.real for el in C_List]
        Li = [el.imag for el in C_List]
        plt.plot(Lr, Li, lw=np.exp(-N/7))
